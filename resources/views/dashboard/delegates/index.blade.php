...
@section('content')
    <delegates-index inline-template>
        <div class="m-portlet">
            <div class="m-portlet__head">
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        <h3 class="m-portlet__head-text">المندوبين</h3>
                    </div>
                </div>
                        ...
                        >
                            ...
                            <table-column show="name" label="الاسم ">
                                <template slot-scope="delegate">
                                    <a :href="'/dashboard/delegates/'+delegate.id" >@{{ delegate.name }}</a>
                            ...
                            <table-column show="budget" label="تاريخ التوثيق  ">
                                <template slot-scope="user">
...
{{--                                    <a @click="deleteItem(user.id, `/dashboard/delegates/${user.id}`, 'userDeleted')" href="javascript:;" class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--pill m-btn--air">--}}
{{--                                        <i class="fa fa-trash-o fa-lg"></i>--}}
{{--                                    </a>--}}

                                    <a :href="'/dashboard/delegates/'+user.id" class="btn btn-outline-info m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--pill m-btn--air">
                                        <i class="fa fa-file-alt fa-lg"></i>
                                    </a>
                                    <a :href="'/dashboard/delegates/'+user.id+'/changeActive'" >
